...
public class EmailSender {
	...
	public ServiceInfoRepository getServiceInfoRepo() {
		return serviceInfoRepo;
	}

	public void setServiceInfoRepo(ServiceInfoRepository serviceInfoRepo) {
	...
	public ModuleInfoRepository getModuleMapRepo() {
		return moduleMapRepo;
	}

	public void setModuleMapRepo(ModuleInfoRepository moduleMapRepo) {
		this.moduleMapRepo = moduleMapRepo;
	}
	...
	public void setLeftServiceName(String leftServiceName) {
		this.leftServiceName = leftServiceName;
	}

	public String getLeftModuleName() {
		return leftModuleName;
	}

	public void setLeftModuleName(String leftModuleName) {
		this.leftModuleName = leftModuleName;
	}
	...
	public void setRightServiceName(String rightServiceName) {
	...
	public boolean isOsyncAdmin() {
		return isOsyncAdmin;
	}

	public void setOsyncAdmin(boolean isOsyncAdmin) {
		this.isOsyncAdmin = isOsyncAdmin;
	}

	public EmailSender(String toEmailAddress, String integId , String contactName,String content, boolean isAdmin) throws Exception {
		super();
		...
		setLeftServiceName(leftServiceInfo.getName());
		setLeftModuleName(leftModuleInfo.getName());
		setRightServiceName(rightServiceInfo.getName());
		setRightModuleName(rightModuleInfo.getName());
		setContactName(contactName);
		setContent(content);
		setOsyncAdmin(isAdmin);
		setIntegId(integId);
		setOsyncId(findById.getOsyncId());

		setToEmailAddress(toEmailAddress);
		setFromEmailAddress(fromEmailAddress);
		setSubject(this.isOsyncAdmin ? "Attention Required for "+this.contactName : "Attention Required!");


	}

	public EmailSender(String subject,String content, String toEmailAddress, String fromEmailAddress ) throws Exception {
		super();
	...
	public boolean sendEmail() {
		String fromAddress = this.getFromEmailAddress();
		String toAddress = this.getToEmailAddress();
		String subject = this.getSubject();
		String content = this.getContent();
		
		Runnable myRunnable = new Runnable() {
			public void run() {
				ZCMailContent zcMailContent = ZCMailContent.getInstance();
				zcMailContent.setHtmlMode(true);
				zcMailContent.setFromEmail(fromAddress);
				zcMailContent.setIsAsync(true);
				zcMailContent.setContent(content);
				zcMailContent.setSubject(subject);
				zcMailContent.setToEmail(toAddress);
				try {
					ZCMail.getInstance().sendMail(zcMailContent);
				} catch (Exception e) {
	...
	public boolean sendEmailThrougMail() {
		...
		try {
			...
			payloadJson.put("fromAddress", "andrew@oapps.xyz");
	...
	public String getMessageContent()
	{
		...
		String customerContent = "<table align=\"center\" width=\"100%\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">\n" +
				...
				"    <strong>Greetings from OApps!</strong> <br><br>Sync between "+this.leftServiceName+" ("+this.leftModuleName+") and "+this.rightServiceName+" ("+this.rightModuleName+") has been stopped due to "+this.content+".<br><br>";
